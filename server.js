...
function User(socket, pseudo) {
    ...
    this.move = function(tab) {
        ...
        if (this.dep == 1 && tab[this.y][this.x - 1] != 0) //L
            ...
            this.x += 1;
        else if (this.dep == 4 && (tab[this.y + 1] == undefined || tab[this.y + 1][this.x] != 0)) //Dfunction Game() {
            this.y += 1;
        else if (this.dep != 0) {
...
io.on('connection', function(socket) {
    ...
    socket.on('disconnect', function() {
        ...
        if (game) {
            remove_matching(game.players, user);
            end_game(game, 'Player disconnect...');
        }
        if (user)
